...
from src.core.registry import registry as _cms_registry  # noqa: E402
from src.core.security import hash_password  # noqa: E402
from src.core.templates import admin_templates, get_frontend_env  # noqa: E402
...
_COMP_SRC_DIR = _COMP_SRC / "com_contact"

# i18n
_translator.load_domain("com_contact", _COMP_SRC_DIR / "i18n")

# Admin templates
_ADMIN_TEMPLATES = _COMP_SRC_DIR / "templates"
if not any(
    isinstance(ldr, FileSystemLoader) and str(_ADMIN_TEMPLATES) in ldr.searchpath
    for ldr in getattr(admin_templates.loader, "loaders", [])
):
    admin_templates.loader.loaders.append(FileSystemLoader(str(_ADMIN_TEMPLATES)))

# Frontend templates
_FRONTEND_TEMPLATES = _COMP_SRC_DIR / "templates"
_frontend_env = get_frontend_env("default")
if not any(
    isinstance(ldr, FileSystemLoader) and str(_FRONTEND_TEMPLATES) in ldr.searchpath
    for ldr in getattr(_frontend_env.loader, "loaders", [])
):
    _frontend_env.loader.loaders.append(FileSystemLoader(str(_FRONTEND_TEMPLATES)))

from src.components.com_contact import admin as _contact_admin  # noqa: E402
from src.components.com_contact import web_views as _contact_web  # noqa: E402
from src.main import app as _cms_app  # noqa: E402

# Register component and web view in the global registry
_cms_registry.register("com_contact", "src.components.com_contact")
_cms_registry.register_web_view("com_contact", "index", _contact_web.index)

if not any(
    getattr(r, "path", "").startswith("/admin/com_contact")
    for r in _cms_app.routes
):
    _cms_app.include_router(_contact_admin.router)
